...
describe('RemoveDataComponent', () => {
  ...
  it('test: show contents of flexible object - component & template', async() => {
    listener.next(true);
    //@ts-ignore
    component.show();
    fixture.detectChanges();
    ...
    expect((component as any).propertyNames()?.length).toEqual(propertyNamesToExpect.length);
    expect((component as any).types()?.length).toEqual(typesToExpect.length);   
    ...
    for(let i = 0; i < properties.length; i++) {
      expect((properties[i].nativeElement as HTMLSpanElement).textContent).toContain(propertyNamesToExpect[i]);
    }

    for(let i = 0; i < types.length; i++) {
      expect((types[i].nativeElement as HTMLSpanElement).textContent).toContain(typesToExpect[i]);
    }
  });
